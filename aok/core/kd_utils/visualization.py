...
def plot_filtered_seafloor_photons(
    filtered_seafloor_subsurface_dataset,
    sea_photon_dataset,
    sea_surface_height,
    output_path,
):
    ...
    sea_surface_x_axis_bins = np.linspace(
        filtered_seafloor_subsurface_dataset["relative_AT_dist"].min(),
    ...
    ax.scatter(
        ...
        alpha=0.15,
        edgecolors="none",
        label="Subsurface ATL03 Photons",
    )

    # Overlay the seafloor elevation data as points
    ax.plot(
        filtered_seafloor_subsurface_dataset["relative_AT_dist"],
        filtered_seafloor_subsurface_dataset["seafloor_elevation"],
        linewidth=0.8,
        c="b",
        alpha=0.4,
        label="Seafloor Elevation",
    )
...
def plot_convex_hulls(photon_dataset, target_beam_ids, convex_hulls, convex_hull_areas):
    ...
    for lat_bin, hull_points in convex_hulls.items():
        ...
        ax.fill(
            hull_points[hull.vertices, 0],
            hull_points[hull.vertices, 1],
            alpha=0.3,
            label=f"Bin {lat_bin}",
        )
        ...
        ax.text(
            centroid[0],
            centroid[1],
            f"{convex_hull_areas[lat_bin]:.2f}",
...
def plot_photon_quality_flags(
    output_path, timestamp, sea_photon_dataset, target_beam_ids
):
    """
    ...
    """
    dataset = sea_photon_dataset[
        sea_photon_dataset["beam_id"].isin(target_beam_ids)
    ].copy()
    if dataset.empty:
        return
...
def plot_kd_photons(
    ...
    Kd_DF_MergedDistance,
):
    # filter beam type
    # photon_dataset
    subsurface_photon_dataset = subsurface_photon_dataset[
        subsurface_photon_dataset["beam_id"].isin(target_beam_ids)
    ]
    Kd_DF_MergedDistance = Kd_DF_MergedDistance[
        Kd_DF_MergedDistance["beam_id"].isin(target_beam_ids)
    ]

    hlims = [-45, 5]
    fig, ax1 = plt.subplots(figsize=(10, 6))
    ax1.scatter(
        subsurface_photon_dataset["relative_AT_dist"],
        subsurface_photon_dataset["photon_height"],
        s=1.5,
        c="k",
        alpha=0.2,
        edgecolors="none",
        label="Subsurface ATL03 Photon Height",
    )
    ...
    if "seafloor_elevation" in subsurface_photon_dataset.columns:
        ax1.plot(
            subsurface_photon_dataset["relative_AT_dist"],
            subsurface_photon_dataset["seafloor_elevation"],
            linewidth=0.8,
            c="b",
            alpha=0.4,
            label="Seafloor Elevation",
    ...
    ax2 = ax1.twinx()
    ax2.scatter(
        Kd_DF_MergedDistance["relative_AT_dist"],
        Kd_DF_MergedDistance["kd"],
        label="Kd values",
        color="r",
        alpha=0.6,
    )
    ...
    fig.tight_layout()
    plt.savefig(
        os.path.join(OutputPath, f"{timestamp}_IS2_subsurface_kd_2.jpg"),
        dpi=400,
        format="jpeg",
    )
    plt.show()
...
def plot_bin_polygon_data(
    ...
    for polygon in polygons:
        plt.gca().add_patch(
            plt.Polygon(polygon, edgecolor="red", facecolor="none", linewidth=1)
        )
