...
def apply_optional_histogram_quality_filter(
    ...
    """
    Optional histogram quality check:
    keep along-track bins only if the target depth-band photon ratio is >= min_ratio.
    """
    ...
    quality_df = (
        pd.DataFrame(
            {"lat_bins": lat_bin_keys, "sea_surface_height": sea_surface_height}
        )
        .dropna(subset=["sea_surface_height"])
        .copy()
    )
    if quality_df.empty:
    ...
    for _, row in quality_df.iterrows():
        ...
        bin_data = binned_dataset_sea_surface[
            binned_dataset_sea_surface["lat_bins"] == lat_bin
        ]
        if bin_data.empty:
            ratios.append(0.0)
            continue
        ...
        total_underwater = int(underwater_mask.sum())
        if total_underwater == 0:
            ratios.append(0.0)
            continue

        band_mask = underwater_mask & (depth >= depth_min) & (depth <= depth_max)
        ratios.append(float(band_mask.sum()) / float(total_underwater))

    quality_df["hist_quality_ratio"] = ratios
    valid_bins = set(
        quality_df.loc[
            quality_df["hist_quality_ratio"] >= min_ratio, "lat_bins"
        ].tolist()
    )
    return subsurface_photon_dataset[
        subsurface_photon_dataset["lat_bins"].isin(valid_bins)
    ].copy()
...
def apply_optional_surface_sigma_filter(
    ...
    """
    Optional Gaussian surface sigma filter:
    discard along-track bins if fitted surface sigma exceeds sigma_max.
    """
    if subsurface_photon_dataset.empty:
        return subsurface_photon_dataset
    ...
    sigma_rows = []
    for lat_bin, surface in zip(lat_bin_keys, sea_surface_height, strict=False):
        if pd.isna(surface):
            continue
        bin_data = binned_dataset_sea_surface[
            binned_dataset_sea_surface["lat_bins"] == lat_bin
        ...
        if len(peak_data) > 10:
            _, sigma = norm.fit(peak_data["photon_height"])
        else:
            sigma = np.nan
        sigma_rows.append((lat_bin, sigma))

    if not sigma_rows:
        return subsurface_photon_dataset

    sigma_df = pd.DataFrame(sigma_rows, columns=["lat_bins", "surface_sigma"])
    valid_bins = set(
        sigma_df.loc[sigma_df["surface_sigma"] <= sigma_max, "lat_bins"].tolist()
    )
    return subsurface_photon_dataset[
        subsurface_photon_dataset["lat_bins"].isin(valid_bins)
    ].copy()
...
def apply_optional_refraction_correction(
    ...
    if subsurface_photon_dataset.empty or (
        not required_cols.issubset(set(subsurface_photon_dataset.columns))
    ):
        return subsurface_photon_dataset
    ...
    corrected = subsurface_photon_dataset.merge(
        surface_df, on="lat_bins", how="left"
    ).copy()
    ...
    ref_elev = pd.to_numeric(corrected["ref_elevation"], errors="coerce").to_numpy(
        dtype=float
    )
    ref_az = pd.to_numeric(corrected["ref_azimuth"], errors="coerce").to_numpy(
        dtype=float
    )
    z = pd.to_numeric(corrected["photon_height"], errors="coerce").to_numpy(dtype=float)
    ws = pd.to_numeric(corrected["sea_surface_height"], errors="coerce").to_numpy(
        dtype=float
    )
...
def apply_sea_surface_flattening(
    ...
    surface_df = (
        pd.DataFrame(
            {"lat_bins": lat_bin_keys, "sea_surface_height_local": sea_surface_height}
        )
        .dropna(subset=["sea_surface_height_local"])
        .copy()
    )
    if surface_df.empty:
        return subsurface_photon_dataset
    ...
    surface_df["big_bin"] = (
        surface_df["lat_bins_num"].astype(int) // big_bin_size
    ).astype(int)
    mean_surface = (
    ...
    surface_df["sea_surface_flattening_offset"] = (
        surface_df["sea_surface_height_bigbin_mean"]
        - surface_df["sea_surface_height_local"]
    )

    flattened = subsurface_photon_dataset.copy()
    flattened["lat_bins_num"] = pd.to_numeric(flattened["lat_bins"], errors="coerce")
    flattened["big_bin"] = (
        flattened["lat_bins_num"].fillna(-1).astype(int) // big_bin_size
    ).astype(int)
    flattened = flattened.merge(
        surface_df[
            [
                "lat_bins",
                "sea_surface_height_local",
                "sea_surface_height_bigbin_mean",
                "sea_surface_flattening_offset",
            ]
        ],
        on="lat_bins",
        how="left",
    ...
    flattened["photon_height"] = flattened["photon_height"] - flattened[
        "sea_surface_flattening_offset"
    ].fillna(0.0)
...
def create_spatial_index(gebco_paths):
    ...
    for i, path in enumerate(gebco_paths):
        with rasterio.Env(GTIFF_SRS_SOURCE="EPSG"):
            ...
            idx.insert(
                i, (bounds.left, bounds.bottom, bounds.right, bounds.top), obj=path
            )
...
def get_relevant_rasters_using_index(lons, lats, raster_data_dict, spatial_index):
    ...
    relevant_rasters = [
        (raster_data_dict[path][0], raster_data_dict[path][1])
...
def process_seafloor_data(
    sea_photon_dataset, gebco_paths, Ignore_Subsurface_Height_Thres
):
    """
    ...
    relevant_rasters = get_relevant_rasters_using_index(
        lons, lats, raster_data_dict, spatial_index
    )

    # Get seafloor elevation for all points
    sea_photon_dataset["seafloor_elevation"] = get_seafloor_elevation(
        lons, lats, relevant_rasters
    )

    # Calculate depth (assuming seafloor_elevation is negative below sea level)
    # If your elevation is positive below sea level, remove the negative sign
    sea_photon_dataset["depth"] = -sea_photon_dataset["seafloor_elevation"]

    # Filter out points:
    # 1. below the seafloor (photon_height > seafloor_elevation)
    # 2. where depth is less than 6 meters (depth >= 6)
    filtered_sea_photon_dataset = sea_photon_dataset[
        (sea_photon_dataset["photon_height"] > sea_photon_dataset["seafloor_elevation"])
        & (sea_photon_dataset["depth"] >= Ignore_Subsurface_Height_Thres)
    ]
...
def load_atl24_points(atl24_file_path):
    """
    Load ATL24 point dataset and normalize to columns: longitude, latitude, seafloor_elevation_atl24.
    """
    if (not atl24_file_path) or (not os.path.exists(atl24_file_path)):
        return pd.DataFrame(
            columns=["longitude", "latitude", "seafloor_elevation_atl24"]
        )
    ...
    if lower_path.endswith((".csv", ".txt")):
        ...
        try:
            ...
            if (
                "longitude" not in atl24_df.columns
                or "latitude" not in atl24_df.columns
            ) and ("geometry" in atl24_gdf.columns):
    ...
    out_df = (
        ...
        .dropna(subset=["longitude", "latitude", "atl24_raw_bathy"])
        .copy()
    )
...
def process_seafloor_data_atl24(
    ...
    """
    Match ATL24 bathymetry points to photons, then filter below seafloor and shallow bins.
    Returns filtered dataset and number of matched photons.
    """
    ...
    distances, indices = tree.query(
        photon_coords, k=1, distance_upper_bound=max_match_distance_deg
    )
    ...
    matched_dataset.loc[valid_match, "seafloor_elevation"] = atl24_points[
        "seafloor_elevation_atl24"
    ].to_numpy()[indices[valid_match]]
    ...
    filtered_dataset = matched_dataset[
        (matched_dataset["photon_height"] > matched_dataset["seafloor_elevation"])
        & (matched_dataset["depth"] >= Ignore_Subsurface_Height_Thres)
    ]
...
def get_subsurface_photon(
    ...
    #     get_sea_surface_height_static(binned_dataset_sea_surface, subsurface_thresh)
    # adaptive threshold
    (
        sea_surface_height,
        sea_surface_label,
    ...
    if apply_refraction_correction:
        ...
        if apply_post_refraction_refit:
            ...
            rebinned_corrected = horizontal_vertical_bin_dataset(
            ...
            (
                ...
                solo_sea_surface_label,
                subsurface_photon_dataset,
            ) = get_sea_surface_height_adaptive(rebinned_corrected)
    ...
    if apply_flattening:
        lat_bin_keys = list(
            binned_dataset_sea_surface.groupby(
