...
def apply_optional_histogram_quality_filter(
    ...
    for _, row in quality_df.iterrows():
        ...
        ref_mask = (
            underwater_mask
            & (depth >= reference_depth_min)
            & (depth <= reference_depth_max)
        )
...
def apply_sea_surface_flattening(
    ...
    if "height_bins" in flattened.columns:
        ...
        orig_bins = (
            pd.to_numeric(flattened["height_bins"], errors="coerce").dropna().unique()
        )
        if len(orig_bins) >= 2:
...
def discard_shallow_seabed_bins(sea_photon_dataset, min_depth_m):
    ...
    """
    return sea_photon_dataset[sea_photon_dataset["depth"] >= min_depth_m].copy()
...
def process_seafloor_data(
    ...
    return discard_shallow_seabed_bins(
        ...
        try:
            ...
            with h5py.File(atl24_file_path, "r") as f:
                for beam in beams:
                    ...
                    grp = f[beam]
                    if not all(
                        k in grp for k in ("lon_ph", "lat_ph", "ortho_h", "class_ph")
                    ):
                        continue
                        ...
                        pd.DataFrame(
                            {
                                ...
                                "seafloor_elevation_atl24": grp["ortho_h"][:][
                                    bathy_mask
                                ].astype(float),
                            }
                        )
...
def process_seafloor_data_atl24(
    ...
    filtered_dataset = discard_shallow_seabed_bins(
        above_floor, abs(Ignore_Subsurface_Height_Thres)
    )  # step 14
...
def get_subsurface_photon(
    ...
    filtered_seafloor_subsurface_photon_dataset = (
        filtered_seafloor_subsurface_photon_dataset.copy()
    )
    if sigma_map:
