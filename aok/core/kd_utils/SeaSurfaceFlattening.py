...
########################################
# readATL03_github
IS2_atl03_mds, IS2_atl03_attrs, IS2_atl03_beams = read_granule(
    ATL03_h5_file_path, ATTRIBUTES=True
)
...
for gtx in sorted(IS2_atl03_beams[0:1]):
    ...
    island_interp1d_model = scipy.interpolate.interp1d(
        segment_lat[:], Segment_Is_Land_Labels[:], fill_value="extrapolate"
    )
    ...
    ref_elev_interp1d_model = scipy.interpolate.interp1d(
        segment_lat[:], ref_elev[:], fill_value="extrapolate"
    )
    # apply interpid model
    ph_ref_elev = ref_elev_interp1d_model(lat_ph[:])

    # Ref_azimuth on a per photon level (assign seg ref_azimuth to photons)
    ref_azimuth_interp1d_model = scipy.interpolate.interp1d(
        segment_lat[:], ref_azimuth[:], fill_value="extrapolate"
    )
    ...
    geoid_interp1d_model = scipy.interpolate.interp1d(
        segment_lat[:], geoid[:], fill_value="extrapolate"
    )
    ...
    binned_dataset_sea_surface = horizontal_vertical_bin_dataset(
        sea_photon_dataset, horizontal_res, vertical_res
    )
...
df_sea_surface["big_bins"] = (
    df_sea_surface["small_bins"] // 50
)  # 50 10 m bins in each 500 m bin


######################
# Step 2: Calculate Mean Sea_surface_height value for Each 500-m Big Bin
mean_sea_surface_elevation_bigBin = (
    df_sea_surface.groupby("big_bins")["sea_surface_height"].mean().reset_index()
)
mean_sea_surface_elevation_bigBin.rename(
    columns={"sea_surface_height": "mean_sea_surface_height_big_bins"}, inplace=True
)

# Prepare subsurface_photon_dataset for merging
subsurface_photon_dataset["big_bins"] = (
    subsurface_photon_dataset["lat_bins"].astype(int) // 50
)

# Merge the mean elevation data with the subsurface_photon_dataset
subsurface_photon_dataset = subsurface_photon_dataset.merge(
    mean_sea_surface_elevation_bigBin, on="big_bins", how="left"
)

# Merge sea_surface_height data with subsurface_photon_dataset based on the small_bins
subsurface_photon_dataset = subsurface_photon_dataset.merge(
    df_sea_surface[["small_bins", "sea_surface_height"]],
    left_on="lat_bins",
    right_on="small_bins",
    how="left",
)
...
subsurface_photon_dataset["adjusted_photon_height"] = subsurface_photon_dataset[
    "photon_height"
] - (
...
subsurface_photon_dataset["Dif_photon_height"] = (
    subsurface_photon_dataset["mean_sea_surface_height_big_bins"]
    - subsurface_photon_dataset["sea_surface_height"]
)
######################################
...
sea_surface_x_axis_bins = np.linspace(
    relative_seg_dist.min(), relative_seg_dist.max(), len(sea_surface_height)
)


mean_sea_surface_x_axis_bins = np.linspace(
    relative_seg_dist.min(),
    relative_seg_dist.max(),
    len(mean_sea_surface_elevation_bigBin),
)
...
x_index_within_zoom = (sea_photon_dataset.relative_AT_dist >= zoom_xlim[0]) & (
    sea_photon_dataset.relative_AT_dist <= zoom_xlim[1]
)
...
subsurface_x_index_within_zoom = (
    subsurface_photon_dataset.relative_AT_dist >= zoom_xlim[0]
) & (subsurface_photon_dataset.relative_AT_dist <= zoom_xlim[1])
subsurface_zoomed_relative_AT_dist = subsurface_photon_dataset.relative_AT_dist[
    subsurface_x_index_within_zoom
]
zoomed_photon_height_adjusted = subsurface_photon_dataset.adjusted_photon_height[
    subsurface_x_index_within_zoom
]
zoomed_photon_height_Dif = subsurface_photon_dataset.Dif_photon_height[
    subsurface_x_index_within_zoom
]
...
ax.scatter(
    sea_photon_dataset.relative_AT_dist,
    sea_photon_dataset.photon_height,
    s=10,
    c="k",
    alpha=0.15,
    edgecolors="none",
...
ax.plot(
    sea_surface_x_axis_bins,
    [x - 0.5 for x in sea_surface_height],
    linewidth=0.8,
    color="#DD571C",
    label="0.5 m Below Surface Peak",
)
ax.plot(
    sea_surface_x_axis_bins,
    [x - 1 for x in sea_surface_height],
    linewidth=0.8,
    color="#FDA172",
    label="1 m Below Surface Peak",
)
ax.plot(
    sea_surface_x_axis_bins,
    [x - 2 for x in sea_surface_height],
    linewidth=0.8,
    color="#FCAE1E",
    label="2 m Below Surface Peak",
)
...
axins = inset_axes(
    ...
    bbox_to_anchor=(
        ...
        0.4,
        0.4,
    ),  # The numbers are x0, y0, width, and height in percentages of the figure size.
...
axins.scatter(
    zoomed_relative_AT_dist,
    zoomed_photon_height,
    s=10,
    c="k",
    alpha=0.15,
    edgecolors="none",
    label="Zoomed ATL03 Photons",
)

axins.scatter(
    subsurface_zoomed_relative_AT_dist,
    zoomed_photon_height_adjusted,
    s=10,
    c="r",
    alpha=0.15,
    edgecolors="none",
    label="Zoomed Adjusted ATL03 Photons",
)


axins.scatter(
    subsurface_zoomed_relative_AT_dist,
    zoomed_photon_height_Dif,
    s=20,
    c="g",
    alpha=0.15,
    edgecolors="none",
    label="Zoomed Dif ATL03 Photons",
)
...
zoomed_indices = [
    index
    for index, x in enumerate(sea_surface_x_axis_bins)
    if zoom_xlim[0] <= x <= zoom_xlim[1]
]
...
axins.plot(
    zoomed_sea_surface_x,
    zoomed_sea_surface_height,
    linewidth=1,
    color="b",
    label="Zoomed Surface Peak",
)
axins.plot(
    zoomed_sea_surface_x,
    [x - 0.5 for x in zoomed_sea_surface_height],
    linewidth=1,
    color="#DD571C",
    label="0.5 m Below Zoomed Surface Peak",
)
axins.plot(
    zoomed_sea_surface_x,
    [x - 1 for x in zoomed_sea_surface_height],
    linewidth=1,
    color="#FDA172",
    label="1 m Below Zoomed Surface Peak",
)
axins.plot(
    zoomed_sea_surface_x,
    [x - 2 for x in zoomed_sea_surface_height],
    linewidth=1,
    color="#FCAE1E",
    label="2 m Below Zoomed Surface Peak",
)

# mean sea surface height at 500m bins
axins.plot(
    mean_sea_surface_x_axis_bins,
    mean_sea_surface_elevation_bigBin,
    linewidth=1,
    color="r",
    label="Zoomed Surface Peak 500m bin",
)
...
rec_hlims = [-10, 3]
rectangle = Rectangle(
    (zoom_xlim[0], rec_hlims[0]),
    zoom_xlim[1] - zoom_xlim[0],
    rec_hlims[1] - rec_hlims[0],
    linewidth=1,
    edgecolor="r",
    facecolor="none",
    linestyle="--",
