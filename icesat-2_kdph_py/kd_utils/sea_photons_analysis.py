...
def get_sea_surface_height_static(binned_data, threshold):
    """
    Calculate sea surface height and filter subsurface photons using adaptive detection.
...
def get_sea_surface_height_adaptive(binned_data):
    ...
    for k, v in data_groups.items():
        ...
        # Wider window for Gaussian fit to capture wave effects
        peak_data = v[(v['photon_height'] > surface_height - 1.0) & 
                     (v['photon_height'] < surface_height + 1.0)]
        if len(peak_data) > 10:
            mu, sigma = norm.fit(peak_data['photon_height'])
        else:
            mu, sigma = surface_height, 0.2  # More conservative default

        # Stricter threshold: 3σ below mean, plus minimum depth
        # Waves can extend beyond 2.5σ, especially in rough conditions. 
        # 3σ is a common statistical cutoff for excluding outliers
        adaptive_threshold = min(mu - 3.0 * sigma, mu - 1.0)  # Ensure at least 1m below
    ...
    for i, (k, v) in enumerate(data_groups.items()):
        if not np.isnan(final_sea_surface_height[i]):
            peak_data = v[(v['photon_height'] > final_sea_surface_height[i] - 1.0) & 
                         (v['photon_height'] < final_sea_surface_height[i] + 1.0)]
            if len(peak_data) > 10:
                mu, sigma = norm.fit(peak_data['photon_height'])
                adaptive_threshold = min(mu - 3.0 * sigma, mu - 1.0)
            else:
                adaptive_threshold = final_sea_surface_height[i] - 1.0  # Stricter default
